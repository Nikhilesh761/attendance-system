...
SCHEDULE_PAGE = BASE_STYLE + "{{ nav|safe }}" + """
<h1>Class Schedule</h1>
<p>Configure each class period and when the automatic attendance scan should start.</p>

<h2>Add period</h2>
<form method="post" action="/schedule/add">
  <input type="text" name="label" placeholder="Period 1 / Mathematics" required>
  <input type="time" name="start_time" required>
  <input type="time" name="end_time" required>
  <input type="time" name="scan_time" required>
  <input type="number" name="scan_duration_sec" value="60" min="1" max="600" required> seconds
  <input type="submit" value="Add Period">
</form>

<h2>Configured periods</h2>
{% if periods %}
<table>
<tr><th>Period</th><th>Class time</th><th>Scan time</th><th>Duration</th><th>Action</th></tr>
{% for p in periods %}
<tr>
<td>{{ p.label }}</td><td>{{ p.start_time }} – {{ p.end_time }}</td><td>{{ p.scan_time }}</td><td>{{ p.scan_duration_sec }} sec</td>
<td>
<form method="post" action="/schedule/delete/{{ p.id }}" style="display:inline" onsubmit="return confirm('Delete this period from the schedule? Historical attendance will not be deleted.');">
...
{% else %}<p>No periods configured yet.</p>{% endif %}
...
def schedule_add():
    try:
        ...
        duration = int(request.form.get("scan_duration_sec", "60"))
        if not label:
