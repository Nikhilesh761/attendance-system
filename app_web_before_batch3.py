...
BASE_STYLE = """
<style>
  :root{--bg:#070707;--panel:#0d0d0d;--panel2:#111;--line:#252525;--muted:#8b8b8b;--text:#f5f5f5;--accent:#ff2020;--good:#39d98a;--warn:#ffb020;}
  *{box-sizing:border-box}
  body{font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;max-width:1180px;margin:0 auto;padding:28px 22px 60px;background:var(--bg);color:var(--text);line-height:1.5}
  h1{font-size:30px;letter-spacing:-.5px;margin:22px 0 8px} h2{font-size:18px;margin:28px 0 12px} h3{margin:0}
  p{color:#aaa}.muted{color:var(--muted)}
  nav{display:flex;gap:8px;flex-wrap:wrap;align-items:center;margin-bottom:18px}
  nav a{color:#ddd;text-decoration:none;padding:8px 10px;border:1px solid transparent;border-radius:8px;font-size:13px} nav a:hover{border-color:#333;background:#111;color:#fff}
  nav a:last-child{margin-left:auto}
  hr{border:0;border-top:1px solid #1b1b1b}
  .stats{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;margin:18px 0}
  .stat-box{background:linear-gradient(145deg,#101010,#0a0a0a);border:1px solid var(--line);border-radius:12px;padding:18px}
  .stat-box .num{font-size:30px;font-weight:800;letter-spacing:-1px}.stat-box .label{font-size:11px;color:#888;text-transform:uppercase;letter-spacing:1.2px;margin-top:3px}
  .card{background:var(--panel);border:1px solid var(--line);border-radius:14px;padding:20px;margin:14px 0}
  .grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:14px}
  .period-card,.log-card{background:#0c0c0c;border:1px solid #222;border-radius:12px;padding:16px}
  .badge{display:inline-block;padding:4px 8px;border-radius:999px;background:#181818;border:1px solid #2a2a2a;color:#bbb;font-size:10px;text-transform:uppercase;letter-spacing:1px}
  .badge.good{color:#9ff0c6;border-color:#1e573e;background:#0c2118}.badge.warn{color:#ffd48a;border-color:#5b4215;background:#211807}.badge.red{color:#ff9b9b;border-color:#5b1e1e;background:#210d0d}
  .progress{height:9px;background:#1b1b1b;border-radius:999px;overflow:hidden;margin:9px 0 4px}.progress>div{height:100%;background:var(--accent);border-radius:999px}
  table{border-collapse:separate;border-spacing:0;width:100%;margin-top:12px;overflow:hidden;border:1px solid #252525;border-radius:10px;background:#0b0b0b}
  th,td{border-bottom:1px solid #222;padding:11px 12px;text-align:left;font-size:13px} th{background:#111;color:#aaa;text-transform:uppercase;letter-spacing:.8px;font-size:10px} tr:last-child td{border-bottom:0}
  button,input[type=submit]{background:var(--accent);color:#fff;border:0;padding:10px 15px;border-radius:8px;cursor:pointer;font-size:13px;font-weight:700;letter-spacing:.3px} button:hover,input[type=submit]:hover{filter:brightness(1.08)} button.danger{background:#641010}
  input[type=text],input[type=password],input[type=time],input[type=number],input[type=date],select{padding:10px 11px;background:#111;border:1px solid #333;color:#fff;border-radius:8px;font:inherit} input:focus,select:focus{outline:2px solid #552020;border-color:#a22}
  .form-row{display:flex;gap:10px;flex-wrap:wrap;align-items:end}.field{display:flex;flex-direction:column;gap:6px}.field label{font-size:11px;text-transform:uppercase;letter-spacing:.8px;color:#999}
  .consent-box{background:#121212;border:1px solid #5b1e1e;padding:14px;border-radius:10px;margin:14px 0}.empty{border:1px dashed #333;border-radius:12px;padding:30px;text-align:center;color:#777}
  a{color:#ddd}.danger-link{color:#ff8585}
  @media(max-width:800px){body{padding:18px 14px 40px}.stats{grid-template-columns:repeat(2,1fr)}nav a:last-child{margin-left:0}.form-row{display:grid;grid-template-columns:1fr}.form-row input,.form-row select{width:100%}}
  @media(max-width:480px){.stats{grid-template-columns:1fr 1fr}.stat-box .num{font-size:24px}h1{font-size:24px}table{display:block;overflow-x:auto;white-space:nowrap}}
</style>
"""
...
def camera_worker(url):
    ...
    completed_schedule_date = datetime.now().strftime("%Y-%m-%d")

    try:
        while True:
            current_date_key = datetime.now().strftime("%Y-%m-%d")
            if current_date_key != completed_schedule_date:
                completed_schedule_keys.clear()
                completed_schedule_date = current_date_key
            with camera_lock:
                if not camera_state["running"]:
                    break
            ...
            if active_mode is None and now - last_schedule_check >= SCHEDULE_CHECK_INTERVAL:
                ...
                if scan_period:
                    scan_key = (now_dt.strftime("%Y-%m-%d"), int(scan_period["id"]))
                    if scan_key in completed_schedule_keys:
                        scan_period = None
                ...
                if scan_period:
                    ...
                    scan_dt = datetime.combine(
                        now_dt.date(),
                        datetime.strptime(scan_period["scan_time"], "%H:%M").time(),
                    )
                    scan_end_dt = scan_dt.timestamp() + int(scan_period["scan_duration_sec"])
                    active_end = max(now, min(now + int(scan_period["scan_duration_sec"]), scan_end_dt))
                    prepared_students = _prepare_students(get_all_students())
                    logged_this_scan = set()
                    pending_export = False
                    last_recognition = 0.0
            ...
            if active_mode is not None and active_end is not None and now >= active_end:
                ...
                if active_mode == "scheduled" and active_key is not None:
                    completed_schedule_keys.add(active_key)
...
def manual_report():
    ...
    total = len(present)+len(absent); pct = round(len(present)/total*100,1) if total else 0
    html=BASE_STYLE+"""{{ nav|safe }}<h1>{{ label }}</h1><p>Manual attendance report · {{ date }}</p><div class="stats"><div class="stat-box"><div class="num">{{ pct }}%</div><div class="label">Attendance</div></div><div class="stat-box"><div class="num">{{ present|length }}</div><div class="label">Present</div></div><div class="stat-box"><div class="num">{{ absent|length }}</div><div class="label">Absent</div></div><div class="stat-box"><div class="num">{{ total }}</div><div class="label">Total</div></div></div><div class="grid"><div class="card"><h2 style="margin-top:0">Present</h2>{% if present %}<table><tr><th>Name</th><th>ID</th><th>Time</th><th>Confidence</th></tr>{% for r in present %}<tr><td>{{ r.name }}</td><td>{{ r.student_id }}</td><td>{{ r.timestamp.split('T')[1][:8] if 'T' in r.timestamp else r.timestamp }}</td><td>{{ r.confidence }}</td></tr>{% endfor %}</table>{% else %}<div class="empty">No students were recognized.</div>{% endif %}</div><div class="card"><h2 style="margin-top:0">Absent</h2>{% if absent %}<table><tr><th>Name</th><th>ID</th></tr>{% for r in absent %}<tr><td>{{ r.name }}</td><td>{{ r.student_id }}</td></tr>{% endfor %}</table>{% else %}<div class="empty">No absentees.</div>{% endif %}</div></div>"""
    return render_template_string(html,label=label,date=date,present=present,absent=absent,total=total,pct=pct)
...
def schedule_add():
    try:
        ...
        app.logger.exception("Schedule add failed")
...
def period_report(period_id):
    ...
    period = get_period(period_id)
    if not period: return redirect(url_for("schedule_page", err="Period not found."))
    ...
    html = BASE_STYLE + """{{ nav|safe }}<div style="display:flex;justify-content:space-between;gap:15px;align-items:end;flex-wrap:wrap"><div><h1>{{ period.label }}</h1><p>{{ period.start_time }} — {{ period.end_time }} · Automatic scan {{ period.scan_time }}</p></div><form method="get" class="form-row"><div class="field"><label>Date</label><input type="date" name="date" value="{{ date }}"></div><input type="submit" value="VIEW"></form></div>
    <div class="stats"><div class="stat-box"><div class="num">{{ pct }}%</div><div class="label">Attendance</div></div><div class="stat-box"><div class="num">{{ present|length }}</div><div class="label">Present</div></div><div class="stat-box"><div class="num">{{ absent|length }}</div><div class="label">Absent</div></div><div class="stat-box"><div class="num">{{ total }}</div><div class="label">Total</div></div></div>
    <div class="card"><div class="progress"><div style="width:{{ pct }}%"></div></div></div>
    <div class="grid"><div class="card"><h2 style="margin-top:0">Present</h2>{% if present %}<table><tr><th>Name</th><th>ID</th><th>Time</th><th>Confidence</th></tr>{% for r in present %}<tr><td>{{ r.name }}</td><td>{{ r.student_id }}</td><td>{{ r.timestamp.split('T')[1][:8] if 'T' in r.timestamp else r.timestamp }}</td><td>{{ r.confidence }}</td></tr>{% endfor %}</table>{% else %}<div class="empty">No students were recognized for this period/date.</div>{% endif %}</div><div class="card"><h2 style="margin-top:0">Absent</h2>{% if absent %}<table><tr><th>Name</th><th>ID</th></tr>{% for r in absent %}<tr><td>{{ r.name }}</td><td>{{ r.student_id }}</td></tr>{% endfor %}</table>{% else %}<div class="empty">No absentees.</div>{% endif %}</div></div>
    """
    return render_template_string(html,period=period,date=date,present=present,absent=absent,pct=pct,total=total)
...
def home():
    ...
    today_rows = [r for r in log if (r.get("session_date") or r.get("timestamp", "")[:10]) == today]
    present_ids = {r["student_id"] for r in today_rows}
    total = len(students)
    present_count = len(present_ids)
    absent_count = max(total - present_count, 0)
    overall_pct = round((present_count / total * 100), 1) if total else 0

    period_cards = []
    for period in get_periods(enabled_only=True):
        rows = [r for r in today_rows if r.get("period_id") == period["id"]]
        ...
        period_cards.append({
            ...
            "scan_time": period["scan_time"], "present": pcount,
            "absent": max(total - pcount, 0),
            "pct": round(pcount / total * 100, 1) if total else 0,
            "scanned": scanned,
        })

    absent_students = [s for s in students if s[0] not in present_ids]
    html = BASE_STYLE + """{{ nav|safe }}
    <div style="display:flex;justify-content:space-between;gap:20px;align-items:end;flex-wrap:wrap">
      <div><h1>Today's Attendance</h1><p style="margin-top:0">Live classroom attendance overview for <b>{{ date }}</b>.</p></div>
      <a href="/live"><button>OPEN LIVE CAMERA</button></a>
    </div>
    <div class="stats">
      <div class="stat-box"><div class="num">{{ overall_pct }}%</div><div class="label">Attendance today</div></div>
      <div class="stat-box"><div class="num">{{ present_count }}</div><div class="label">Present</div></div>
      <div class="stat-box"><div class="num">{{ absent_count }}</div><div class="label">Not present</div></div>
      <div class="stat-box"><div class="num">{{ total }}</div><div class="label">Total students</div></div>
    </div>
    <div class="card"><div style="display:flex;justify-content:space-between;gap:15px;align-items:center"><div><h2 style="margin:0">Period Overview</h2><div class="muted">Each period is isolated into its own attendance session.</div></div><a href="/schedule">Manage schedule →</a></div></div>
    {% if period_cards %}<div class="grid">
      {% for p in period_cards %}<div class="period-card">
        <div style="display:flex;justify-content:space-between;gap:8px"><div><div class="muted" style="font-size:11px;text-transform:uppercase">{{ p.start_time }} — {{ p.end_time }}</div><h3>{{ p.label }}</h3></div>{% if p.scanned %}<span class="badge good">Scanned</span>{% else %}<span class="badge warn">No data</span>{% endif %}</div>
        <div style="margin-top:16px;font-size:25px;font-weight:800">{{ p.pct }}%</div><div class="progress"><div style="width:{{ p.pct }}%"></div></div>
        <div class="muted">{{ p.present }} present · {{ p.absent }} absent · scan {{ p.scan_time }}</div>
        <div style="margin-top:14px"><a href="/period_report/{{ p.id }}?date={{ date }}">Open period report →</a></div>
      </div>{% endfor %}
    </div>{% else %}<div class="empty">No class periods configured. <a href="/schedule">Add your schedule</a>.</div>{% endif %}
    <div class="card"><h2 style="margin-top:0">Today's Recognized Students</h2>
      {% if today_rows %}<table><tr><th>Student</th><th>ID</th><th>Period</th><th>Time</th><th>Confidence</th></tr>{% for r in today_rows %}<tr><td>{{ r.name }}</td><td>{{ r.student_id }}</td><td>{{ r.period_label or 'Manual Scan' }}</td><td>{{ r.timestamp.split('T')[1][:8] if 'T' in r.timestamp else r.timestamp }}</td><td>{{ r.confidence }}</td></tr>{% endfor %}</table>{% else %}<div class="empty">No attendance has been recorded today.</div>{% endif %}
    </div>
    {% if absent_students %}<div class="card"><h2 style="margin-top:0">Not Present Today</h2><div class="grid">{% for sid,name,_ in absent_students %}<div class="period-card"><b>{{ name }}</b><div class="muted">{{ sid }}</div></div>{% endfor %}</div></div>{% endif %}
    """
    return render_template_string(html,total=total,present_count=present_count,absent_count=absent_count,overall_pct=overall_pct,date=today,period_cards=period_cards,today_rows=today_rows,absent_students=absent_students)
...
def full_log():
    ...
    rows = log
    if date_filter:
        rows = [r for r in rows if (r.get("session_date") or r.get("timestamp", "")[:10]) == date_filter]
    if period_filter:
        rows = [r for r in rows if (r.get("period_label") or "Manual Scan") == period_filter]
    if search:
        rows = [r for r in rows if search in r.get("name", "").lower() or search in r.get("student_id", "").lower()]
    periods = sorted({r.get("period_label") or "Manual Scan" for r in log})
    html = BASE_STYLE + """{{ nav|safe }}
    <div style="display:flex;justify-content:space-between;align-items:end;gap:15px;flex-wrap:wrap"><div><h1>Full Attendance Log</h1><p style="margin-top:0">Search and filter every recorded recognition event.</p></div><a href="/export/excel"><button>EXPORT EXCEL</button></a></div>
    <div class="card"><form method="get" class="form-row">
      <div class="field"><label>Date</label><input type="date" name="date" value="{{ date_filter }}"></div>
      <div class="field"><label>Period</label><select name="period"><option value="">All periods</option>{% for p in periods %}<option value="{{ p }}" {% if p==period_filter %}selected{% endif %}>{{ p }}</option>{% endfor %}</select></div>
      <div class="field"><label>Student / ID</label><input type="text" name="q" value="{{ search }}" placeholder="Search"></div><input type="submit" value="FILTER"><a href="/log">Reset</a>
    </form></div>
    <div class="card"><div class="muted">{{ rows|length }} record(s) shown</div>{% if rows %}<table><tr><th>Date</th><th>Time</th><th>Student</th><th>ID</th><th>Period</th><th>Type</th><th>Confidence</th></tr>{% for r in rows %}<tr><td>{{ (r.session_date or r.timestamp[:10]) }}</td><td>{{ r.timestamp.split('T')[1][:8] if 'T' in r.timestamp else r.timestamp }}</td><td>{{ r.name }}</td><td>{{ r.student_id }}</td><td>{{ r.period_label or 'Manual Scan' }}</td><td>{{ r.session_type or 'scheduled' }}</td><td>{{ r.confidence }}</td></tr>{% endfor %}</table>{% else %}<div class="empty">No records match these filters.</div>{% endif %}</div>
    """
    return render_template_string(html,rows=rows,date_filter=date_filter,period_filter=period_filter,search=search,periods=periods)


@app.route("/percent")
@login_required(role="teacher")
def percent_view():
    try: total_sessions = max(0, int(request.args.get("sessions", 1)))
    except (TypeError,ValueError): total_sessions = 1
    try: threshold = max(0, min(100, int(request.args.get("threshold", 75))))
    except (TypeError,ValueError): threshold = 75
    pct_data = get_attendance_percentage(total_sessions)
    pct_data.sort(key=lambda r: (-r["attendance_pct"], r["name"].lower()))
    defaulters = [r for r in pct_data if r["attendance_pct"] < threshold]
    avg = round(sum(r["attendance_pct"] for r in pct_data)/len(pct_data),1) if pct_data else 0
    html = BASE_STYLE + """{{ nav|safe }}
    <h1>Attendance Percentage</h1><p>Visual attendance standing across the number of sessions you specify as held.</p>
    <div class="card"><form method="get" class="form-row"><div class="field"><label>Total sessions held</label><input type="number" min="0" name="sessions" value="{{ total_sessions }}"></div><div class="field"><label>Defaulter threshold</label><input type="number" min="0" max="100" name="threshold" value="{{ threshold }}"></div><input type="submit" value="UPDATE"></form></div>
    <div class="stats"><div class="stat-box"><div class="num">{{ avg }}%</div><div class="label">Class average</div></div><div class="stat-box"><div class="num">{{ total_sessions }}</div><div class="label">Sessions held</div></div><div class="stat-box"><div class="num">{{ pct_data|length }}</div><div class="label">Students</div></div><div class="stat-box"><div class="num">{{ defaulters|length }}</div><div class="label">Below threshold</div></div></div>
    <div class="card">{% if pct_data %}<div class="grid">{% for r in pct_data %}<div class="period-card"><div style="display:flex;justify-content:space-between;gap:10px"><div><b>{{ r.name }}</b><div class="muted">{{ r.student_id }}</div></div><span class="badge {% if r.attendance_pct >= threshold %}good{% else %}red{% endif %}">{{ r.attendance_pct }}%</span></div><div class="progress"><div style="width:{{ r.attendance_pct }}%"></div></div><div class="muted">{{ r.sessions_present }} present · {{ r.sessions_absent }} absent</div></div>{% endfor %}</div>{% else %}<div class="empty">No enrolled students.</div>{% endif %}</div>
    """
