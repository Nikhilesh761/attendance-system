...
class FFmpegCapture:
    """Small VideoCapture-compatible wrapper around ffmpeg.

    This avoids OpenCV's RTSP transport negotiation entirely. ffmpeg reads
    the phone RTSP stream and emits JPEG frames; the service decodes those
    JPEGs into the same BGR numpy arrays used by the recognition pipeline.
    """
    ...
    def _start(self):
        # UDP is tried first because this phone RTSP endpoint has previously
        # produced a stream outside OpenCV. TCP is a fallback.
        transports = ["udp", "tcp"]
        for transport in transports:
            try:
                ...
                # Give ffmpeg a moment to negotiate the stream.
                ...
                if got_frame is not None:
                    self.proc = p
                    self.latest = got_frame
                    self.started = True
                    self.thread = threading.Thread(
                        target=self._reader, args=(buf,), daemon=True
                    )
                    self.thread.start()
                    return
    ...
    def _reader(self, initial_buf):
        buf = bytearray(initial_buf)
        while not self.stop_event.is_set() and self.proc is not None:
            try:
