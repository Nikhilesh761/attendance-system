...
class CameraState:
    ...
    def _open_capture(self):
        ...
        # IMPORTANT:
        # This phone RTSP server may use UDP RTP. Do NOT force TCP first.
        # Try UDP first, then TCP, then OpenCV's normal backend.
        attempts = [
            ("udp", "rtsp_transport;udp|stimeout;5000000|max_delay;500000"),
            ("tcp", "rtsp_transport;tcp|stimeout;5000000|max_delay;500000"),
            ("default", None),
        ]
        ...
        for name, opts in attempts:
            ...
            try:
                ...
                if hasattr(cv2, "CAP_PROP_OPEN_TIMEOUT_MSEC"):
                    params += [cv2.CAP_PROP_OPEN_TIMEOUT_MSEC, 8000]
                if hasattr(cv2, "CAP_PROP_READ_TIMEOUT_MSEC"):
                    params += [cv2.CAP_PROP_READ_TIMEOUT_MSEC, 5000]

                backend = cv2.CAP_FFMPEG
                cap = (
                    cv2.VideoCapture(url, backend, params)
                    if params else
        ...
        # Restore the environment after the attempts.
        if old_opts is None:
            os.environ.pop("OPENCV_FFMPEG_CAPTURE_OPTIONS", None)
        else:
            os.environ["OPENCV_FFMPEG_CAPTURE_OPTIONS"] = old_opts

        # Last fallback: let OpenCV select the available backend.
        try:
